...
{
	return baseReflectivity + (1 - baseReflectivity) * pow(1 - max(0.0, dot(V, H)), 5);
}

const float pi = 3.1415926535897932384626433832795;
const vec3 baseReflectivity = vec3(0.4);
...
float ShlickBeckmann(vec3 N, vec3 X, float k)
{
	...
	return NdotX / max(0.0000001, NdotX * (1.0 - k) + k);
}
...
vec3 Specular_CookTorence(float alpha, vec3 N, vec3 V, vec3 L)
{
	...
	return numer / max(denom, 0.0000001);
}
